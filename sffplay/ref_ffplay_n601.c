...
typedef struct VideoState {
    ...
    /**
     * PacketQueue满了后读队列需要停止，此时需要contine_read_thread
     * seek_stream后队列中的packet无效了，此时需要使用contine_read_thread唤醒read_thread
     */ 
...
/**
 * 计算videoClock和master_clock的diff用于时钟对齐
*/
...
retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            /**
             * 视频暂停就直接跳转到display，因此虽然暂停了但循环仍然继续，只是没有frame_queue_next，依旧展示同一帧
             */ 
            if (is->paused)
                goto display;
